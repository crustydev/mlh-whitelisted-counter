...
use whitelist::cpi::accounts::{
    AddWallet, CheckWallet, CreateWhitelist, RemoveWallet, SetAuthority,
};
...
pub mod counter {
    ...
    pub fn create_counter_whitelist(ctx: Context<CreateCounterWhitelist>) -> Result<()> {
        ...
        _ = whitelist::cpi::set_authority(set_authority_ctx, ctx.accounts.counter.key());
    ...
    pub fn update_counter(ctx: Context<UpdateCounter>) -> Result<()> {
        ...
        let cpi_ctx = ctx.accounts.into_check_wallet_context().with_signer(signer);
...
pub struct CreateCounter<'info> {
    ...
    #[account(
        ...
        bump, payer = authority, space = Counter::LEN
    )]
...
impl<'info> CreateCounterWhitelist<'info> {
    pub fn into_create_whitelist_context(
        &self,
    ) -> CpiContext<'_, '_, '_, 'info, CreateWhitelist<'info>> {
...
impl Counter {
    const LEN: usize =
        DISCRIMINATOR_LENGTH + (PUBKEY_LENGTH) * 2 + UNSIGNED64_LENGTH + UNSIGNED8_LENGTH;
}
